...
def validate_sanity_tests(run_path: Path) -> Tuple[List[str], List[str]]:
    ...
    if not sanity_path.exists():
        ...
        print("❌ sanity_tests.json not found")
        return fail_reasons, warnings
    ...
    try:
        ...
        if shuffled_auc is None:
            ...
            print("❌ Missing shuffled_labels_auc")
        else:
            ...
            if shuffled_auc > threshold:
                ...
                print(f"❌ Shuffled labels AUC: {shuffled_auc:.3f} (FAIL - should be ≤ {threshold})")
            else:
                print(f"✓ Shuffled labels AUC: {shuffled_auc:.3f} (PASS)")
        
        # Validate future shift test
        if future_auc is None:
            fail_reasons.append("sanity_tests.json missing future_shift_auc")
            print("❌ Missing future_shift_auc")
        else:
            ...
            if future_auc > threshold:
                ...
                print(f"❌ Future shift AUC: {future_auc:.3f} (FAIL - should be ≤ {threshold})")
            else:
                print(f"✓ Future shift AUC: {future_auc:.3f} (PASS)")
        
    except Exception as e:
        fail_reasons.append(f"Failed parsing sanity_tests.json: {e}")
        print(f"❌ Error parsing sanity_tests.json: {e}")
    ...
    try:
        ...
        for field in REQUIRED_METRICS_FIELDS:
            if field in metrics:
                ...
                fail_reasons.append(f"metrics.json missing field: {field}")
...
def validate_abstain_stats(run_path: Path) -> Tuple[List[str], List[str]]:
    ...
    if not abstain_path.exists():
        ...
        print("❌ abstain_stats.json not found")
        return fail_reasons, warnings
    ...
    try:
        ...
        print("\nRequired fields:")
        for field in REQUIRED_ABSTAIN_STATS_FIELDS:
            if field in stats:
                value = stats[field]
                print(f"  ✓ {field}: {value}")
            else:
                fail_reasons.append(f"abstain_stats.json missing field: {field}")
                print(f"  ❌ {field}: MISSING")
        
    except Exception as e:
        fail_reasons.append(f"Failed parsing abstain_stats.json: {e}")
        print(f"❌ Error parsing abstain_stats.json: {e}")
...
def validate_oos_samples(run_path: Path) -> Tuple[List[str], List[str]]:
    ...
    if not pred_path.exists():
        ...
        print("❌ predictions_oos.parquet not found")
        return fail_reasons, warnings
    
    if not PYARROW_AVAILABLE:
        warnings.append("pyarrow not installed; cannot count OOS rows")
        print("⚠️  pyarrow not installed - cannot verify row count")
        return fail_reasons, warnings
    ...
    try:
        ...
        if oos_rows < MIN_OOS_ROWS_WARN:
            ...
            print(f"⚠️  WARNING: Only {oos_rows} OOS samples (< {MIN_OOS_ROWS_WARN} recommended)")
        else:
            print(f"✓ Sufficient OOS samples (≥ {MIN_OOS_ROWS_WARN})")
        
    except Exception as e:
        fail_reasons.append(f"Failed reading predictions_oos.parquet: {e}")
        print(f"❌ Error reading predictions_oos.parquet: {e}")
...
def main():
    ...
    if not run_path.exists():
        print(f"\n❌ FAIL: Run path does not exist: {run_path}")
        sys.exit(1)
    ...
    failures, warnings = validate_artifacts(run_path)
    all_failures.extend(failures)
    all_warnings.extend(warnings)
    
    failures, warnings = validate_sanity_tests(run_path)
    all_failures.extend(failures)
    all_warnings.extend(warnings)
    
    failures, warnings = validate_metrics(run_path)
    all_failures.extend(failures)
    all_warnings.extend(warnings)
    
    failures, warnings = validate_abstain_stats(run_path)
    all_failures.extend(failures)
    all_warnings.extend(warnings)
    
    failures, warnings = validate_oos_samples(run_path)
    all_failures.extend(failures)
    all_warnings.extend(warnings)
    
    # Print summary and exit
    success = print_summary(all_failures, all_warnings)
